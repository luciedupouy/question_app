...
function LongAnswerPage({ userId, resetUserId }) {
    ...
    useEffect(() => {
        const fetchQuestion = async () => {
            try {
                const response = await axios.get('https://question-app-4iey.vercel.app/get_long_answer_question');
    ...
    const handleConfirmSubmit = async () => {
        try {
            await axios.post('https://question-app-4iey.vercel.app/submit_long_answer', {
                id: userId,
