...
function Identification({ onSuccessfulIdentification }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const checkResponse = await axios.post('https://question-app-4iey.vercel.app/check_identity', { email, id });
      
      if (checkResponse.status === 200) {
        console.log('Identity verified, fetching answers');
        try {
          const answersResponse = await axios.get(`https://question-app-4iey.vercel.app/get_answers/${id}`);
